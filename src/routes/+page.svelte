...
const renderChart = () => {
  ...
  articles.forEach(article => {
    if (typeCounts.hasOwnProperty(article.businessType)) typeCounts[article.businessType]++;
  ...
  chart = new Chart(chartCanvas, {
    ...
    data: {
      ...
      datasets: [{
        ...
        data: categories.map(cat => typeCounts[cat]),
...
const resetForm = () => {
  businessName = ''; businessType = ''; businessDescription = ''; businessOwner = ''; businessSubtitle = ''; editingId = null;
...
const submitForm = async () => {
  ...
  const payload = {
    ...
    businessSubtitle: businessSubtitle.trim(),
...
const editArticle = (article) => {
  ...
  businessSubtitle = article.businessSubtitle || '';
  days.forEach(day => {
    const slot = article.businessHours?.[day]?.[0];
    hoursConfig[day] = slot
      ? { open: slot.open, close: slot.close, closed: false }
...
{#if !isAuthenticated}
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
    <div class="w-full max-w-sm rounded-xl border bg-white p-8 shadow-xl">
      ...
      <div class="mt-5 flex flex-col gap-3">
        <input
          class="w-full rounded border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400"
          type="text"
        ...
        />
        {#if loginError}
          <p class="text-xs text-red-600">{loginError}</p>
        {/if}
        <button
          class="mt-1 rounded bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-700 transition-colors"
        ...
        <a
            href="/home"
            class="block text-center w-full rounded px-4 py-2 text-sm font-medium text-slate-600 border border-slate-200 hover:bg-slate-50 transition-colors">
        Go Back Home
        </a>
      </div>
    </div>
...
<!-- Main Page (always rendered, overlaid when not authenticated) -->
<div class="min-h-screen bg-slate-50 text-slate-900" class:blur-sm={!isAuthenticated} class:pointer-events-none={!isAuthenticated} class:select-none={!isAuthenticated}>
    ...
    <div class="mt-6 rounded-lg border bg-white p-4">
      ...
      <form class="mt-3" on:submit|preventDefault={submitForm}>
        <div class="grid gap-3 sm:grid-cols-2">
          ...
          <input class="w-full rounded border px-3 py-2 text-sm" placeholder="Short tagline (4–5 words)" bind:value={businessSubtitle}/>
        </div>
          ...
          {#each articles as article}
            <li class="flex items-center justify-between px-4 py-3 hover:bg-slate-50">
              <div class="overflow-hidden mr-4">
                ...
                {#if article.businessSubtitle}
                    <p class="text-xs text-slate-400">{article.businessSubtitle}</p>
                {/if}
              </div>
